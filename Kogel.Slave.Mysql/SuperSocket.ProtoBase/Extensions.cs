...
namespace SuperSocket.ProtoBase
{
    ...
    public static class Extensions
    {
        ...
        public static string GetString(this ReadOnlySequence<byte> buffer, Encoding encoding)
        {
            ...
            {
                return encoding.GetString(buffer.First.Span.ToArray());
            }
            ...
            while (enumerator.MoveNext())
            {
                ...
                int chars = decoder.GetChars(bytes: current.Span.ToArray(), byteIndex: 0, byteCount: current.Span.Length, span2.ToArray(), 0, flush: false);
                stringBuilder.Append(new string((chars == span2.Length) ? span2.ToArray() : span2.Slice(0, chars).ToArray()));
            }
        ...
        public static int Write(this IBufferWriter<byte> writer, ReadOnlySpan<char> text, Encoding encoding)
        {
            ...
            while (!completed)
            {
                ...
                //char[] chars, int charIndex, int charCount, byte[] bytes, int byteIndex, int byteCount, bool flush, out int charsUsed, out int bytesUsed, out bool completed
                encoder.Convert(chars: text.ToArray(), 0, text.Length, span.ToArray(), 0, span.Length, false
