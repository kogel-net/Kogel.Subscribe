...
namespace SuperSocket.Channel
{
    ...
    public class TcpPipeChannel<TPackageInfo> : PipeChannel<TPackageInfo>
    {
        ...
        {
            return _socket.Receive(memory.ToArray(), SocketFlags.None);
        }
        ...
        protected override async ValueTask<int> SendOverIOAsync(ReadOnlySequence<byte> buffer, CancellationToken cancellationToken)
        {
            ...
            {
                return await _socket.SendAsync(new List<ArraySegment<byte>> { GetArrayByMemory(buffer.First) }, SocketFlags.None).ConfigureAwait(continueOnCapturedContext: false);
            }
